...
- [[データウェアハウス]]
- [[データカタログ]]
- [[MLOps]] - ML パイプラインのオーケストレーションに Airflow を活用
- [[Apache Hadoop]] - Hadoop ジョブのワークフロー管理に Airflow を活用
- [[Apache Kafka]] - Kafka へのデータ書き込み・読み込みを Airflow DAG で管理
- [[dbt]] - Airflow で dbt ジョブをスケジューリングする構成が広く使われる
- [[データ品質]] - データ品質チェックタスクを Airflow パイプラインに組み込む
- [[データリネージ]] - OpenLineage と Airflow の統合でパイプラインのリネージを記録
- [[ETL]] — Airflow は ETL/ELT パイプラインのオーケストレーションツールとして最も普及
- [[Docker]] - Airflow 自体や DAG タスクをコンテナで実行
- [[DataOps]] - Airflow は DataOps のオーケストレーション基盤として機能
- [[Prefect]] — Python ネイティブな代替オーケストレーター
- [[Pipeline as Code]] — Airflow の DAG 定義は Pipeline as Code の実践
- [[モダンデータスタック]] — モダンデータスタックのオーケストレーション層を担う
