...
- [[OLAP]] - SQL で分析クエリを実行するオンライン分析処理
- [[OLTP]] - SQL でトランザクションを処理するオンライン業務処理
- [[NoSQL]] - 非リレーショナルデータベースの総称．スキーマ柔軟性と水平スケールが特徴
- [[BigQuery]] — Google Cloud の SQL ベースクラウド DWH
- [[クエリオプティマイザ]] — SQL の実行計画を最適化するエンジンコンポーネント
- [[リレーショナルデータベース]] — SQL を操作言語として使う RDB の解説
- [[アトミックトランザクション]] — SQL の BEGIN/COMMIT/ROLLBACK で実装する原子性
- [[ルックアップ]] — SQL の JOIN 句を用いたルックアップテーブルの活用
