...
現在Hadoopではなく別のツール(オブジェクトストレージ，[[Apache Spark]]など)を使うことが増えているが，分散型ストレージと分散処理の概念は以前として重要である．
...
Java で書かれた分散ファイルシステムで，大規模データを複数ノードにまたがって格納する．大規模なデータワークロードを効率的に保存・処理するための仕組みである．
