...
Kappa アーキテクチャは 2014 年に Jay Kreps（Apache Kafka 共同開発者、Confluent 創業者）が提唱したデータ処理アーキテクチャ。[[Lambdaアーキテクチャ]] のバッチ層を廃止し、**全てのデータをストリームとして処理**するシンプルな設計を採用する。ログベースのストレージ（Kafka 等）に全履歴を保持することで、バッチ再処理をストリーム処理のリプレイで代替する。
