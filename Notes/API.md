...
- [[Swagger]] - REST API の設計・ドキュメント化・テストツール．OpenAPI Specification の実装
- [[疎結合]] - API はコンポーネント間の疎結合を実現する基本手段
- [[マイクロサービスアーキテクチャ]] - サービス間通信に REST/gRPC/GraphQL を活用
- [[モノリスアーキテクチャ]] - モジュラーモノリスでは内部 API でモジュール間を分離
- [[ベゾスAPI司令]] - Amazon 全社に API ファースト設計を義務化した指令
- [[SSO]] - API への認証・認可に OAuth/OIDC が使われる
- [[HTTP・HTTPS]] - Web API の基盤プロトコル
- [[REST API]] — REST アーキテクチャスタイルの詳細
- [[GraphQL]] — クライアント指定クエリの詳細
- [[Webhook]] — イベント駆動プッシュ通知の詳細
- [[RPC・gRPC]] — Google の高性能 RPC フレームワーク
