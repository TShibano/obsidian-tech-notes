...
- [[Julia]] - 科学技術計算言語（高性能を共通目標とする）
- [[オブジェクト指向プログラミング]] - トレイトベースで OOP の良い部分を取り込んでいる
- [[関数型プログラミング]] - イミュータブル・高階関数・Option/Result など FP の概念を採用
- [[データ指向プログラミング]] - ECS フレームワーク（Bevy）の実装言語として DOD と親和性が高い
- [[Tauri]] - Rust 製クロスプラットフォーム・デスクトップ／モバイルアプリフレームワーク
- [[gpui]] - Zed エディタが開発した GPU アクセラレーション対応 Rust 製 UI フレームワーク
- [[Leptos]] - Rust 製フルスタック・アイソモーフィック Web フレームワーク
- [[Trunk]] - Rust/Wasm Web アプリケーションのビルド・バンドルツール
